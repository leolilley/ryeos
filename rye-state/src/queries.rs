...
pub fn set_facets(
    ...
    for (key, value) in facets {
        conn.execute(
            "INSERT OR REPLACE INTO thread_facets (thread_id, key, value, updated_at) \
             VALUES (?, ?, ?, ?)",
            rusqlite::params![thread_id, key, value.as_bytes(), now],
        )
        .with_context(|| format!("set_facet key={key}"))?;
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn set_and_get_facets() {
        let db = test_db();
        set_facets(&db, "T-1", &[("k1".into(), "v1".into()), ("k2".into(), "v2".into())])
            .unwrap();

        let facets = get_facets(&db, "T-1").unwrap();
        assert_eq!(facets.len(), 2);
        assert_eq!(facets[0].key, "k1");
        assert_eq!(String::from_utf8_lossy(&facets[0].value), "v1");

        set_facets(&db, "T-1", &[("k1".into(), "updated".into())]).unwrap();
        let facets = get_facets(&db, "T-1").unwrap();
        assert_eq!(facets.len(), 2);
        let k1 = facets.iter().find(|f| f.key == "k1").unwrap();
        assert_eq!(String::from_utf8_lossy(&k1.value), "updated");
    }
