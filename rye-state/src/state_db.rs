...
impl StateDb {
    ...
    pub fn create_chain(
        ...
        if let Err(e) = projection::project_thread_snapshot(
            &self.projection,
            &snapshot,
            chain_root_id,
        ) {
            tracing::warn!(
                chain_root_id,
                error = %e,
                "projection failed after CAS write; drift will be reconciled at startup"
            );
        }
    ...
    pub fn append_events(
        ...
        for event in &result.events {
            if let Err(e) = projection::project_event(&self.projection, event) {
                tracing::warn!(
                    chain_root_id,
                    thread_id,
                    chain_seq = event.chain_seq,
                    error = %e,
                    "projection failed for event; drift will be reconciled at startup"
                );
            }
        }

        for update in &snapshot_updates {
            if let Err(e) = projection::project_thread_snapshot(&self.projection, &update.new_snapshot, chain_root_id) {
                tracing::warn!(
                    chain_root_id,
                    thread_id = %update.thread_id,
                    error = %e,
                    "projection failed for snapshot update; drift will be reconciled at startup"
                );
            }
        }
