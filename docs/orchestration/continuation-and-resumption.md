...
The seed user message appended to `resume_messages` is generated by a **continuation directive** rather than being hardcoded. This lets directives control how they're resumed.

### Resolution

Step 3.5 of `execute()` resolves the continuation directive:

1. `directive.get("continuation_directive")` — per-directive override
2. Falls back to `rye/agent/continuation` — system default

The directive receives these inputs:

| Input | Description |
|-------|-------------|
| `original_directive` | The directive name being continued |
| `original_directive_body` | The directive's body/instructions |
| `previous_thread_id` | The thread being continued from |
| `continuation_message` | User message (for resume) or default handoff instruction |

The interpolated body of the continuation directive becomes the trailing user message in `resume_messages`. `thread_continued` hooks still fire on top of this in the runner.

### Per-Directive Override

A directive can specify its own continuation directive:

```xml
<directive name="my-workflow" version="1.0.0">
  <metadata>
    <continuation_directive>my/custom-continuation</continuation_directive>
    <!-- ... -->
  </metadata>
  <!-- ... -->
</directive>
...
This lets directives control how they're resumed — e.g., a code review directive might re-inject the file list, a deployment directive might re-check state.

### Fallback

If the continuation directive fails to load (not found, parse error), step 3.5 falls back to the raw `continuation_message` string. This ensures handoff never fails due to a missing or broken continuation directive.
...
If a directive doesn't declare these hooks, the handoff still works — the continuation thread gets the trailing messages and the default continuation directive's prompt (which includes the original directive name, body, and a "pick up where you left off" instruction). No summary, no extra context. This is fine for simple directives that don't need it.
