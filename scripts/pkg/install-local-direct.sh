...
case "$sandbox_mode" in
    ...
    enforce)
        if ! bwrap_compatible "$sandbox_backend"; then
            ryeos_term_fail "node sandbox backend '$sandbox_backend' is missing or incompatible"
            ryeos_term_fail "RyeOS enforcement requires Bubblewrap 0.11.0+ with exact help tokens: --bind-fd --ro-bind-fd --argv0"
            if [[ "$sandbox_backend" == "/usr/bin/bwrap" ]]; then
                ryeos_term_info "install a current bubblewrap package that provides /usr/bin/bwrap (Arch: sudo pacman -S bubblewrap)"
            else
                ryeos_term_info "install a compatible binary at the configured path or change '$sandbox_policy' to executable: /usr/bin/bwrap after installing it there"
            fi
