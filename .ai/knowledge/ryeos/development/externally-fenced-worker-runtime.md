<!-- ryeos:signed:2026-09-17T01:07:32Z:2b8eb1586f48a6032d5d0e3be4e74a4c44956a45eddc4d61f12f37da9bf37a75:xCF4VPMoRtBn3B7ElYLrvQP/dEZ8H7en/vcVSZHO2vISjqqmWL4Nss2Ybd4Z2/uyydxGW2WI3MhIXtuq3q7aBw==:741a8bc609b398aaec0685e5aefb682faf5129a66bd192f888d23bb642c18eea -->
```yaml
...
Provider-specific service IDs, regions, image coordinates, lifecycle API
bindings, budgets, and credentials belong to the consuming project or an
installed provider bundle. They are not RyeOS development constants. Railway,
Fly, Modal, a managed container service, or a future deployment backend can
bind this contract only after its own adapter and installed evidence satisfy
the same generic authority requirements.
...
launch must refuse before provider contact.

## Authority split

The source controller owns configured-remote selection, exact workflow intent,
aggregate concurrency policy, and recovery. Each target owns its node identity,
credential-profile home, provider session, target launch, private candidate,
and candidate testimony. Deployment credentials and mutation authority belong
only to an independently admitted source-side lifecycle adapter. They are never
installed on a target or included in a project generation.

Three terminal facts remain separate:
...
3. The external incarnation ended and its slot became reusable.

The target can prove the first two. It cannot prove its own future death or
safe replacement. `service:node/status` therefore cannot satisfy external
host-incarnation cleanup, even if a target returns plausible provider IDs or
digests. The source-side adapter must retain an independent lifecycle receipt
and compose it with, never replace, the existing workflow and candidate
receipts.
...
An accepted lifecycle receipt binds the preconfigured slot, configured remote,
authenticated node boot registration, source launch intent,
provider-observed deployment occurrence, expected image/configuration, and
controller observation generation. It is controller-observed provider evidence
unless the provider supplies independently verifiable attestation.
...
Replacement cannot release capacity until the adapter establishes that the old
logical incarnation is fenced and no stale target can authenticate as the
current slot. Candidate capture does not release the site or credential
profile. Equivalent recorded workflow replay performs no lifecycle mutation
and no model contact.
...
2. Exercise a fake lifecycle adapter with contact counters across stale
   incarnation, ambiguous mutation, overlapping replacement, crash before
