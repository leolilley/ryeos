<!-- rye:signed:2026-02-17T23:54:02Z:4bd2ebf4b6f4f80ea30fc8e3541f1f514c9c5beddd279f971cd5bec70bde7828:0rC22c4uw_ljlf7PrQgF4439xhvDuMSuS8972V7MIBzJnbqXlswgxinth3pbyncBlqUZLgcTD4mly3xA3-_9DA==:440443d0858f0199 -->
...
| ----------------------------- | ------------------------- | --------------------------------------------------------------------------- |
| Check trust                   | `is_trusted(fingerprint)` | Checks `{fp}.pem` exists, then checks `registry.pem` fingerprint           |
| Get key by fingerprint        | `get_key(fingerprint)`    | Returns PEM bytes by fingerprint, falls back to registry key                |
...
| Pin registry key              | `pin_registry_key(pem)`   | Writes `registry.pem` (**no-op if already pinned**)                         |
| Get registry key              | `get_registry_key()`      | Returns registry PEM or `None`                                              |
| List all keys                 | `list_keys()`             | Returns all `.pem` files with fingerprints                                  |

### Trust Lookup Order

1. Check `~/.ai/trusted_keys/{fingerprint}.pem` exists → **trusted**
2. Check `~/.ai/trusted_keys/registry.pem` exists AND its computed fingerprint matches → **trusted**
3. Otherwise → **untrusted**

**Auto-trust:** User's own public key is automatically added to the trust store on first keygen.
...
4. TrustStore.pin_registry_key(registry_key_pem) → writes registry.pem
...
**No fallback or bypass.** Any check failure → `IntegrityError` → execution denied.
