...
Keyed by (kind, extension) to avoid conflicts between extractors that
...
# Global cache: (kind, extension) -> signature format
...
def get_signature_format(
    ...
    """
    Get signature format for a file based on its extension and kind.

    Args:
        file_path: Path to the file
        project_path: Optional project path for extractor discovery
        kind: Kind string (tool, directive, knowledge)
    ...
    format_config = _signature_formats.get((kind, ext))
    ...
    raise ValueError(
        ...
        f"Add it to the {kind} extractor's signature_formats "
...
def _load_signature_formats(
    ...
    """Load signature formats from all extractors, keyed by (kind, ext)."""
    ...
    for extractors_dir in search_paths:
        ...
        for file_path in list(extractors_dir.glob("**/*_extractor.yaml")) + list(
            ...
            # Derive kind from extractor filename: tool_extractor -> tool
