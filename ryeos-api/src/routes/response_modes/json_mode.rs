...
pub struct CompiledJsonMode {
    ...
    /// `source_config` is null/empty, the POST body is parsed as the
    /// handler input directly.
    body_is_json: bool,
}
...
impl CompiledResponseMode for CompiledJsonMode {
    ...
    async fn handle(
        ...
        // Determine input:
        // 1. If source_config is set, interpolate it with path captures.
        // 2. If source_config is null/empty and request.body is json,
        //    parse the POST body as the input directly.
        let input = if !self.source_config_template.is_null()
            && self.source_config_template != serde_json::Value::Object(serde_json::Map::new())
        {
            interpolation::interpolate_path(&self.source_config_template, &ctx.captures)?
        } else if self.body_is_json && !ctx.body_raw.is_empty() {
