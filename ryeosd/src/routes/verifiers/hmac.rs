//! Generic, config-driven HMAC-SHA256 signature verifier.
//!
...
//!
//! `signed_payload` is an ordered list of byte producers (sources +
//! literals). The verifier concatenates the bytes, computes
//! HMAC-SHA256 with the secret, encodes per `signature.encoding`, and
//! constant-time compares against each candidate value extracted from
//! `signature.header` per `signature.select`.
//!
//! `timestamp` (optional) extracts a u64 timestamp value and rejects
//! requests outside `tolerance_secs` drift. `delivery_id` (optional)
//! extracts a unique id used for replay-window dedupe and for
//! downstream metadata. `dedupe` (optional) wires per-route
//! deduplication via the process-wide [`WebhookDedupeStore`].
//!
//! Compile-time fail-closed: if `timestamp` is absent, `dedupe` MUST
//! be present. Otherwise the route has no replay protection at all,
//! which is a configuration mistake, not a default to silently
//! accept.
//!
//! Secrets are read from `secret_env` at compile time. A missing or
//! empty environment variable is a load-time hard fail. Secret
//! bytes are stored once in the compiled verifier; the request path
//! never reads `std::env`.
...
/// One entry in `signed_payload`. Either a literal string or a byte
/// source. Validated at compile time — the flat shape here exists so
/// that `deny_unknown_fields` rejects stray keys loudly; the typed
/// variant choice is built by `compile_signed_payload_item`.
///
/// `template` is accepted at the deserialization layer so the loud
/// rejection of `from=synthesized` happens in the kind-aware
/// validation step (which emits the specific error message rather
/// than a generic "unknown field"). Synthesized templates are only
/// valid under `delivery_id.extract`; the validator rejects them
/// here.
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
...
impl AuthVerifier for HmacVerifier {
    ...
    fn validate_route_config(
        ...
        // Fail-closed: at least one replay-protection mechanism is
        // required. Timestamp window OR dedupe — never neither.
        ...
        // If dedupe is enabled, delivery_id must be too — otherwise
        // there is no key to dedupe on.
...
fn compile_signed_payload_item(
    ...
    // `synthesized` is the only `from` that uses `template`; rejecting
    // it explicitly below means no other variant should ever see a
    // `template` field. Reject early with a kind-aware error.
    ...
    match from {
        ...
        "header" => {
            ...
            let name = raw
                .header
                .as_ref()
                .ok_or_else(|| cfg_err(route_id, &format!("{label} from=header requires `header`")))?;
...
fn parse_template_variable(
    ...
    if let Some(rest) = inner.strip_prefix("header_pair.") {
        // Format: header_pair.<header>.<key>. The header name is
        // lowercase ASCII without dots; split on the first dot from
        // the right is unsafe — use the first dot from the left
        // because keys may contain other characters but not dots.
...
// ── Compiled verifier (request-time) ─────────────────────────────────

struct CompiledHmacVerifier {
    secret_bytes: Vec<u8>,
...
impl CompiledAuthVerifier for CompiledHmacVerifier {
    fn verify(
        &self,
        route_id: &str,
        ...
        // 1. Build signed payload bytes from the configured sources.
        let signed_bytes = match self.build_signed_payload(req) {
            Ok(b) => b,
            Err(reason) => {
        ...
        let raw_header_value = match read_header(req.headers, &self.signature.header) {
            Ok(v) => v,
            Err(reason) => {
        ...
        let candidates: Vec<&str> = match &self.signature.select {
            ...
            SignatureSelect::Prefix { prefix } => {
                let v = raw_header_value.strip_prefix(prefix.as_str()).ok_or_else(|| {
                    tracing::warn!(
                        route_id = route_id,
        ...
        if let Some(ts_spec) = &self.timestamp {
            let v = match self.extract_value(req, &ts_spec.extract, raw_header_value) {
                Ok(v) => v,
                Err(reason) => {
        ...
        let delivery_id = match &self.delivery_id {
            Some(d) => {
                let v = self
                    .extract_value(req, &d.extract, raw_header_value)
                    .map_err(|reason| {
        ...
        Ok(RoutePrincipal {
            id: format!("webhook:{}:{}", VERIFIER_KEY, route_id),
            scopes: vec![],
            verifier_key: "hmac",
            verified: true,
            metadata,
...
impl CompiledHmacVerifier {
    fn build_signed_payload(
        ...
        let mut out: Vec<u8> = Vec::with_capacity(req.body_raw.len() + 64);
        for item in &self.signed_payload {
            match item {
                SignedPayloadItem::Literal(b) => out.extend_from_slice(b),
                SignedPayloadItem::Body => out.extend_from_slice(req.body_raw),
                SignedPayloadItem::Header(name) => {
                    let v = read_header(req.headers, name)?;
                    out.extend_from_slice(v.as_bytes());
                }
                SignedPayloadItem::HeaderPair { header, key } => {
                    let v = read_header(req.headers, header)?;
                ...
                SignedPayloadItem::BodyJsonPath { path } => {
                    let v = extract_body_json_string(req.body_raw, path)?;
                    out.extend_from_slice(v.as_bytes());
                }
            }
        }
    ...
    fn extract_value(
        ...
        match extract {
            ValueExtract::Header(name) => Ok(read_header(req.headers, name)?.to_string()),
            ...
            ValueExtract::BodyJsonPath { path } => {
                Ok(extract_body_json_string(req.body_raw, path)?.to_string())
            }
                ...
                for part in parts {
                    match part {
                        ...
                        TemplatePart::Header(name) => {
                            out.push_str(read_header(req.headers, name)?);
                        }
...
fn extract_body_json_string(body: &[u8], path: &str) -> Result<String, String> {
    ...
    let s = field.as_str().ok_or_else(|| {
        format!("body key '{path}' is not a JSON string")
    })?;
    Ok(s.to_string())
}
...
mod tests {
    ...
    /// `RoutePrincipal` is not `Debug`, so `Result::unwrap_err`
    /// can't format the success arm. Helper that panics on `Ok`.
    fn expect_dispatch_err(
        r: Result<RoutePrincipal, RouteDispatchError>,
    ) -> RouteDispatchError {
        match r {
            Err(e) => e,
            Ok(_) => panic!("expected error, got Ok(principal)"),
        }
    }

    fn with_secret_env<F: FnOnce(&str) -> R, R>(env_var: &str, value: &str, f: F) -> R {
    ...
    // ── Stripe-style end-to-end (header_pair t + literal "." + body)

    #[test]
    fn stripe_style_valid_signature_accepted() {
        with_secret_env("RYEOSD_TEST_HMAC_STRIPE_OK", "stripe-secret", |env| {
            ...
            let v = HmacVerifier;
            let compiled = v.validate_route_config("r1", Some(&cfg)).unwrap();
            ...
            let req = VerifierRequestContext {
            ...
            let principal = compiled.verify("r1", &req, &state).unwrap();
            assert!(principal.verified);
            assert_eq!(principal.verifier_key, "hmac");
            assert_eq!(principal.id, "webhook:hmac:r1");
            assert_eq!(principal.metadata.get("delivery_id").map(String::as_str), Some("evt_1"));
        });
    }

    #[test]
    fn stripe_style_wrong_secret_rejected() {
        with_secret_env("RYEOSD_TEST_HMAC_STRIPE_BAD", "real-secret", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [
                    { "from": "header_pair", "header": "stripe-signature", "key": "t" },
            ...
            let (_tmp, state) = build_test_state();
            let err = expect_dispatch_err(compiled.verify("r1", &req, &state));
            assert!(matches!(err, RouteDispatchError::Unauthorized));
        });
    }

    #[test]
    fn stripe_style_stale_timestamp_rejected() {
        with_secret_env("RYEOSD_TEST_HMAC_STRIPE_STALE", "stripe-secret", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [
                    { "from": "header_pair", "header": "stripe-signature", "key": "t" },
    ...
    // ── GitHub-style end-to-end (body, prefix select, dedupe-only)

    #[test]
    fn github_style_valid_signature_accepted_and_replay_rejected() {
        with_secret_env("RYEOSD_TEST_HMAC_GH_OK", "gh-secret", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
            ...
            let principal = compiled.verify("r1", &req, &state).unwrap();
            assert_eq!(principal.metadata.get("delivery_id").map(String::as_str), Some("abc-123"));
            assert_eq!(
                principal.metadata.get("header.x-github-event").map(String::as_str),
                Some("push")
            );

            // Replay: same delivery id within TTL → Unauthorized.
            let err = expect_dispatch_err(compiled.verify("r1", &req, &state));
            assert!(matches!(err, RouteDispatchError::Unauthorized));
        });
    }

    // ── Slack-style end-to-end (literal "v0:" + ts + ":" + body, synthesized delivery_id)

    #[test]
    fn slack_style_valid_signature_with_synthesized_delivery_id() {
        with_secret_env("RYEOSD_TEST_HMAC_SLACK_OK", "slack-secret", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [
                    { "literal": "v0:" },
            ...
            // Composed: <ts>:<first 20 chars of sig header>
    ...
    fn compile_rejects_missing_auth_config() {
        let err = expect_err(HmacVerifier.validate_route_config("r1", None));
        assert!(format!("{err}").contains("auth_config is required"), "got: {err}");
    }
    ...
    fn compile_rejects_unset_env() {
        ...
        let err = expect_err(HmacVerifier.validate_route_config("r1", Some(&cfg)));
        assert!(format!("{err}").contains("is not set"), "got: {err}");
    }

    #[test]
    fn compile_rejects_empty_signed_payload() {
        with_secret_env("RYEOSD_TEST_HMAC_EMPTY_PAYLOAD", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [],
                "signature": {
    ...
    fn compile_rejects_uppercase_header_name() {
        with_secret_env("RYEOSD_TEST_HMAC_UC_HEADER", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [
    ...
    fn compile_rejects_forbidden_header_name() {
        with_secret_env("RYEOSD_TEST_HMAC_FORBID", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_bad_encoding() {
        with_secret_env("RYEOSD_TEST_HMAC_BAD_ENC", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_select_kind_pair_values_without_key() {
        with_secret_env("RYEOSD_TEST_HMAC_SELECT_NO_KEY", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_select_kind_prefix_without_prefix() {
        with_secret_env("RYEOSD_TEST_HMAC_SELECT_NO_PREFIX", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_select_kind_unknown() {
        with_secret_env("RYEOSD_TEST_HMAC_SELECT_UNK", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_zero_tolerance() {
        with_secret_env("RYEOSD_TEST_HMAC_ZERO_TOL", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_zero_dedupe_ttl() {
        with_secret_env("RYEOSD_TEST_HMAC_ZERO_TTL", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_zero_dedupe_capacity() {
        with_secret_env("RYEOSD_TEST_HMAC_ZERO_CAP", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_no_timestamp_no_dedupe() {
        // Fail-closed: no replay protection at all is rejected.
        with_secret_env("RYEOSD_TEST_HMAC_NO_REPLAY", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_dedupe_without_delivery_id() {
        with_secret_env("RYEOSD_TEST_HMAC_DEDUP_NO_DID", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_synthesized_in_signed_payload() {
        with_secret_env("RYEOSD_TEST_HMAC_SYN_PAYLOAD", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [
    ...
    fn compile_rejects_nested_json_path() {
        with_secret_env("RYEOSD_TEST_HMAC_NESTED_PATH", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_unknown_template_variable() {
        with_secret_env("RYEOSD_TEST_HMAC_BAD_TEMPLATE", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn compile_rejects_unterminated_template() {
        with_secret_env("RYEOSD_TEST_HMAC_UNTERM_TEMPL", "x", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
    ...
    fn template_parser_accepts_known_variables() {
        let parts =
            parse_synthesized_template("r1", "test", "${header.x-foo}-${signature_prefix:8}-tail")
                .unwrap();
        // [Header, Literal("-"), SignaturePrefix(8), Literal("-tail")]
    ...
    fn anonymous_principal_uses_btreemap() {
        // Sanity that BTreeMap import resolves and anonymous works.
        let p = RoutePrincipal::anonymous("x".into(), "test");
        assert_eq!(p.metadata, BTreeMap::new());
    }

    #[test]
    fn dedupe_isolated_per_route() {
        with_secret_env("RYEOSD_TEST_HMAC_DEDUP_ISO", "secret", |env| {
            let cfg = serde_json::json!({
                "secret_env": env,
                "signed_payload": [{ "from": "body" }],
                "signature": {
            ...
            let v = HmacVerifier;
            let compiled_a = v.validate_route_config("route_a", Some(&cfg)).unwrap();
            let compiled_b = v.validate_route_config("route_b", Some(&cfg)).unwrap();
            let body = b"";
            ...
            compiled_a.verify("route_a", &req, &state).unwrap();
            compiled_b.verify("route_b", &req, &state).unwrap();
            // Second hit on route_a → replay.
            let err = expect_dispatch_err(compiled_a.verify("route_a", &req, &state));
            assert!(matches!(err, RouteDispatchError::Unauthorized));
        });
    }
}
