...
fn unwrap_tool_result(status: reqwest::StatusCode, body: &Value, ctx: &str) -> Value {
    ...
    tool_envelope.get("result").cloned().unwrap_or_else(|| {
        panic!("{ctx}: tool envelope missing inner `result`; got: {tool_envelope}")
    })
}
...
async fn tool_identity_public_key_returns_doc() {
    ...
    assert!(
        has_principal,
        "identity doc missing principal_id key: {result}"
    );
...
async fn service_bundle_list_returns_at_least_core() {
    ...
    for entry in bundles {
        assert!(
            ...
            "bundle entry missing name: {entry}"
        );
        assert!(
            entry.get("path").and_then(|v| v.as_str()).is_some(),
            "bundle entry missing path: {entry}"
        );
    }
}
...
async fn service_threads_get_returns_audit_row() {
    ...
    for required in ["thread", "result", "artifacts", "facets"] {
        assert!(
            ...
            "threads.get missing key {required}: {result}"
        );
    }
    ...
    let inner_id = inner
        .get("thread_id")
        .and_then(|v| v.as_str())
        .expect("inner thread_id");
    assert_eq!(
        inner_id, &*tid,
        "round-trip mismatch: queried {tid} got {inner_id}"
    );
}
...
async fn service_threads_get_missing_returns_null() {
    ...
    assert!(
        result.is_null(),
        "missing thread should return null result, got {result}"
    );
}
...
async fn service_threads_chain_returns_chain_for_audit_thread() {
    ...
    if !result.is_null() {
        assert!(
            result.is_object(),
            "threads.chain non-null must be object, got {result}"
        );
    }
}
...
async fn service_events_replay_returns_events_for_audit_thread() {
    ...
    assert!(
        result.get("events").and_then(|v| v.as_array()).is_some(),
...
async fn tool_fetch_resolves_known_service() {
    ...
    assert!(
        obj.get("resolved_path").and_then(|v| v.as_str()).is_some(),
...
async fn tool_fetch_unknown_ref_errors() {
    ...
    assert!(
        status.is_success(),
        "fetch returned HTTP error; body={body}"
    );
    ...
    let inner = tool_envelope
        .get("result")
        .cloned()
        .unwrap_or(tool_envelope.clone());
    let fetch_status = inner
        .get("fetch_status")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    assert!(
...
async fn tool_verify_returns_trusted_for_core_service() {
    let (h, _fixture) =
        start_with_isolated_bundle("tool_verify_returns_trusted_for_core_service").await;
    let (status, body) = exec(
        &h,
...
async fn service_commands_submit_against_audit_thread() {
    ...
    if status.is_success() {
        ...
        assert!(
            result.get("command_id").is_some() || result.get("id").is_some(),
            "command record missing id: {result}"
        );
    } else {
        let body_str = body.to_string().to_lowercase();
        assert!(
            body_str.contains("complet")
                || body_str.contains("status")
                || body_str.contains("thread")
                || body_str.contains("capabilit")
