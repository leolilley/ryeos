...
fn main() {
    ...
    let (qualification, version, revision, build_date, source_date_epoch) = if exact {
        if profile != "release" {
            panic!("an exact contained-oci-hook realization requires Cargo profile `release`");
        }
        (
            "exact".to_owned(),
    ...
    let testimony = format!(
        concat!(
            "schema=ryeos.contained-oci-hook-build.v1\n",
            "qualification={}\n",
            "version={}\n",
            "source_revision={}\n",
            "build_date={}\n",
    ...
    fs::write(out_dir.join("ryeos-contained-oci-hook-build"), testimony)
        .expect("write contained-oci-hook build testimony");
}
...
fn required_exact_value(name: &str) -> String {
    let value = env::var(name).unwrap_or_else(|error| {
        panic!("exact contained-oci-hook build requires explicit {name}: {error}")
    });
    if value.is_empty()
        || value == "unknown"
        || value == "unqualified"
    ...
    {
        panic!("exact contained-oci-hook build received non-canonical {name}");
    }
    value
}
