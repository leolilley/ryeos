...
mod tests {
    ...
    fn guard_old_route_enum_form_is_rejected() {
        // The pre-seat StudioInputRoute serde forms must NOT parse as a
        // route. No dual forms: the old tagged-enum shapes are dead.
        let old_studio_context = json!({ "route": { "type": "studio_context" } });
        let parsed = InputRoute::from_surface_input(Some(&old_studio_context));
        assert!(
            parsed.is_none() || !parsed.unwrap().has_target(),
            "old studio_context form must not yield a targeted route"
        );
