...
async fn main() -> Result<()> {
    ...
    // Discovery and the signed mint use one normalized base and both refuse
    // redirects, so the configured origin cannot move between them.
    ...
    let resp = client
        .post(format!("{}{}", discovered.base_url, MINT_PATH))
