...
core in `crates/clients/base` (`StudioCore`: state, reducer, effects, semantic
...
## Effects — `StudioEffectKind` (18 variants, both sides covered)

Every effect the core emits is executed by an adapter on each renderer. Browser
adapter: `pkg/studio_effects.js`. Terminal adapter:
...
| Effect kind | Web (`studio_effects.js`) | Terminal (`effects.rs`) |
| --- | --- | --- |
...
`StudioEvent` dispatched into `StudioCore`; JS only translates transport and DOM.

## Widgets — `StudioViewVm` variants

The core projects each bound view into a `StudioViewVm` variant; each renderer
draws it. Terminal reference renderers live in
`crates/clients/terminal/src/render/widgets/`.

| Widget (`type`) | Web (`studio_components_workspace.js`) | Terminal reference |
| --- | --- | --- |
...
terminal draws the shared `StudioSceneModel` as a cell grid; the browser draws
the same model with DOM/CSS (and Three.js where a 3D scene is bound). Both
consume the SAME `StudioSceneModel` — the divergence is presentation only, not
...
The binding table is the shared `studio_key_command` in
`crates/clients/base/src/studio/keymap.rs`. Both renderers translate a native
key event into a neutral `StudioKeyEvent` and route it through that function
against the shared `key_context()`:

- Terminal: `crates/clients/terminal/src/app/keys.rs` (crossterm adapter).
- Web: `pkg/studio_shell.js` (DOM `KeyboardEvent` → `StudioKeyEvent`) →
  `wasm.rs::studio_key` → `studio_key_command`. The command-resolution
...
- Native text entry: the input-dock `<textarea>` and the launcher `<input>`
  ...
  listeners in `studio_components_workspace.js` / `studio_components_chrome.js`).
...
`studio_key_command`), because the feed is a cell-grid lens. The shared keymap
...
1. `table` and `sections` widgets were default-cased to "Unknown view" on web,
   ...
   `studio_components_workspace.js` (`tableView` / `sectionsView`), mirroring the
...
2. The web reimplemented the global keymap by hand in `studio_shell.js`
   (hardcoded `keydown` branches). Replaced with a single route through the
   shared `studio_key_command` via `wasm.rs::studio_key`. This also picked up
...
- There is no JavaScript test harness in the web crate and `wasm.rs` compiles
  ...
  DOM-key → `StudioKeyEvent` translation (`studioKeyName`) and the
  table/sections DOM structure against `StudioTableRowVm` / `StudioSectionVm`.
