...
export function captureWorkspaceMotion(root) {
  ...
  root.querySelectorAll(".ryeos-workspace .ryeos-tile[data-tile-id]").forEach((tile) => {
...
function animateSplitBeams(root, snapshot, motionEvents) {
  ...
  layer.className = "ryeos-motion-layer beam-layer";
  layer.setAttribute("aria-hidden", "true");
  root.append(layer);

  for (const target of targets) {
    const tile = root.querySelector(`.ryeos-workspace .ryeos-tile[data-tile-id="${cssEscape(target.tileId)}"]`);
    if (!tile) continue;
    const rect = tile.getBoundingClientRect();
    const beam = document.createElement("i");
    const vertical = target.axis ? target.axis === "horizontal" : rect.height >= rect.width * 0.62;
    beam.className = `ryeos-split-beam ${vertical ? "vertical" : "horizontal"}`;
    if (vertical) {
...
function animateRetainedExits(root, snapshot, currentTileIds) {
  ...
  const layer = document.createElement("div");
  layer.className = "ryeos-motion-layer";
  layer.setAttribute("aria-hidden", "true");
  root.append(layer);

  for (const [tileId, old] of exits) {
    const clone = document.createElement("section");
    clone.className = `ryeos-tile${old.focused ? " focused" : ""} ryeos-tile-ghost`;
    const rect = old.rect;
