//! Transactional outbox helpers for bundle/domain event projections.
...
CREATE INDEX IF NOT EXISTS idx_domain_outbox_claim
    ON domain_outbox_messages(status, next_attempt_at, id);

CREATE INDEX IF NOT EXISTS idx_domain_outbox_event
    ON domain_outbox_messages(event_hash);
"#;

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DomainOutboxMessage {
    pub id: i64,
...
pub fn ensure_domain_outbox_schema(conn: &Connection) -> anyhow::Result<()> {
    conn.execute_batch(OUTBOX_SCHEMA_SQL)
        .context("create domain outbox schema")
}

pub fn enqueue_domain_outbox_message(
    conn: &Connection,
    projection_name: &str,
    sink: &str,
    ...
    validate_domain_identifier("sink", sink)?;
    let now = lillux::time::iso8601_now();
    let payload_json = serde_json::to_string(payload_json)?;
    conn.execute(
        "INSERT INTO domain_outbox_messages (
...
pub fn claim_domain_outbox_messages(
    conn: &mut Connection,
    projection_name: &str,
    sink: &str,
    worker_id: &str,
...
) -> anyhow::Result<Vec<DomainOutboxMessage>> {
    validate_domain_identifier("projection_name", projection_name)?;
    validate_domain_identifier("sink", sink)?;
    validate_domain_identifier("worker_id", worker_id)?;
    if limit == 0 {
        return Ok(Vec::new());
    }
...
pub fn mark_domain_outbox_delivered(
    conn: &Connection,
    id: i64,
    worker_id: &str,
    attempts: u32,
...
pub fn mark_domain_outbox_failed(
    conn: &Connection,
    id: i64,
    worker_id: &str,
    attempts: u32,
...
pub fn get_domain_outbox_message(
    conn: &Connection,
    id: i64,
) -> anyhow::Result<Option<DomainOutboxMessage>> {
...
fn select_outbox_message(
    ...
    .optional()
    .context("read domain outbox message")
}
...
mod tests {
    ...
    fn append_request(chain_id: &str) -> DomainEventAppendRequest {
    ...
    fn outbox_enqueue_claim_deliver_is_idempotent() {
        ...
        let appended =
            append_domain_event(&cas_root, &refs_root, append_request("email_1"), &signer).unwrap();

        let mut conn = Connection::open_in_memory().unwrap();
        ensure_domain_outbox_schema(&conn).unwrap();
        let record = DomainEventRecord {
            event_hash: appended.event_hash.clone(),
            event: appended.event.clone(),
        };
        enqueue_domain_outbox_message(
            &conn,
            "ryeos-email",
            "smtp",
            &record,
        ...
        let claimed = claim_domain_outbox_messages(
            &mut conn,
            "ryeos-email",
            "smtp",
            "worker_1",
        ...
        assert!(mark_domain_outbox_delivered(
            &conn,
            claimed[0].id,
            "worker_1",
            claimed[0].attempts
        ...
        .unwrap());
        let delivered = get_domain_outbox_message(&conn, claimed[0].id)
            .unwrap()
            .unwrap();
    ...
    fn outbox_failed_with_retry_becomes_pending_again() {
        ...
        let signer = TestSigner::default();
        let appended =
            append_domain_event(&cas_root, &refs_root, append_request("email_2"), &signer).unwrap();
        ...
        assert!(mark_domain_outbox_failed(
            &conn,
            claimed[0].id,
            "worker_1",
            claimed[0].attempts,
        ...
        .unwrap());
        let retried = claim_domain_outbox_messages(
            &mut conn,
            "ryeos-email",
            "smtp",
            "worker_2",
    ...
    fn outbox_stale_worker_cannot_finalize_reclaimed_message() {
        ...
        let signer = TestSigner::default();
        let appended =
            append_domain_event(&cas_root, &refs_root, append_request("email_3"), &signer).unwrap();
        let record = DomainEventRecord {
            event_hash: appended.event_hash.clone(),
            event: appended.event.clone(),
        };
        ...
        let first_claim = claim_domain_outbox_messages(
            &mut conn,
            "ryeos-email",
            "smtp",
            "worker_1",
        ...
        .unwrap();
        let second_claim = claim_domain_outbox_messages(
            &mut conn,
            "ryeos-email",
            "smtp",
            "worker_2",
    ...
    fn outbox_max_attempts_moves_failure_to_terminal_failed() {
        ...
        let signer = TestSigner::default();
        let appended =
            append_domain_event(&cas_root, &refs_root, append_request("email_4"), &signer).unwrap();
        let record = DomainEventRecord {
            event_hash: appended.event_hash.clone(),
            event: appended.event.clone(),
        };
        ...
        .unwrap());
        let failed = get_domain_outbox_message(&conn, claimed[0].id)
            .unwrap()
            .unwrap();
        assert_eq!(failed.status, "failed");
        let reclaimed = claim_domain_outbox_messages(
            &mut conn,
            "ryeos-email",
            "smtp",
            "worker_2",
    ...
    fn outbox_expired_final_attempt_is_marked_failed_on_next_claim() {
        ...
        let signer = TestSigner::default();
        let appended =
            append_domain_event(&cas_root, &refs_root, append_request("email_5"), &signer).unwrap();
        let record = DomainEventRecord {
            event_hash: appended.event_hash.clone(),
            event: appended.event.clone(),
        };
