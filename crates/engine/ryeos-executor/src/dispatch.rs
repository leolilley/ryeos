...
pub(crate) async fn dispatch_method(
    ...
    let cap = state.callback_tokens.generate_with_context(
        ...
        hop_verified
            .as_ref()
            .map(|verified| verified.resolved.raw_content_digest.clone())
            .ok_or_else(|| DispatchError::ProjectionInvariant {
                reason: format!(
                    "method callback root `{canonical_ref}` has no verified content digest"
                ),
            })?,
