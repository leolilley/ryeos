...
impl RuntimeCallbackAPI for UdsRuntimeClient {
    ...
    async fn ingest_large_content(
        ...
        let map = request.as_object_mut().ok_or_else(|| {
            CallbackError::Transport(anyhow::anyhow!(
                "runtime.ingest_large_content request must be a JSON object"
            ))
        })?;
        map.insert("thread_id".to_string(), json!(thread_id));
        self.inject_callback_token(&mut request);
        self.rpc
            .request("runtime.ingest_large_content", request)
            .await
            .map_err(Self::map_rpc_error)
    }
    ...
    async fn scrub_large_content(
        ...
        let map = request.as_object_mut().ok_or_else(|| {
            CallbackError::Transport(anyhow::anyhow!(
                "runtime.scrub_large_content request must be a JSON object"
            ))
        })?;
        map.insert("thread_id".to_string(), json!(thread_id));
        self.inject_callback_token(&mut request);
        self.rpc
            .request("runtime.scrub_large_content", request)
            .await
            .map_err(Self::map_rpc_error)
    }
