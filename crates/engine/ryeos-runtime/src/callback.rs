...
pub trait RuntimeCallbackAPI: Send + Sync {
    ...
    /// Stream operator-named bytes into the node's large-object store and
    /// publish the large-content manifest. Manifest-declared runtime
    /// authority (`large_content: [ingest]`); refused by default.
    async fn ingest_large_content(
        &self,
        _thread_id: &str,
        _request: Value,
    ) -> Result<Value, CallbackError> {
        Err(CallbackError::Transport(anyhow::anyhow!(
            "runtime.ingest_large_content callback is not implemented by this client"
        )))
    }
