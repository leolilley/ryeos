...
//! ## Why this exists
//!
//! In the dev tree `bundles/core/.ai/bin/<triple>/ryeos-core-tools` is
//! a symlink to `target/debug/ryeos-core-tools`. The bundle manifest hashes
//! the dereferenced binary, so any `cargo build` cycle that recompiles
//! `ryeos-core-tools` invalidates the manifest. Under
...
/// over stale blobs (typically gigabytes of accumulated `ryeos-core-tools`
...
/// Copy `bundles/core` into `target/test-bundles/<test_name>/`,
...
pub fn isolated_core_bundle(test_name: &str) -> PathBuf {
    ...
    let source = workspace.join("bundles/core");
