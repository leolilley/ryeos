...
pub fn execute_json(raw: &str) -> anyhow::Result<SearchEnvelope> {
    let params: SearchParams = serde_json::from_str(raw).context("parse search params JSON")?;
    ...
    let (provider, results) = search_web(query, num_results, provider)?;
    Ok(SearchEnvelope {
        success: true,
        provider,
...
fn search_web(
    ...
    match provider {
        SearchProvider::Auto => search_web_auto(query, num_results),
        SearchProvider::BingWebRss => {
            search_bing_web_rss(query, num_results).map(|r| ("bing_web_rss".to_string(), r))
        }
        SearchProvider::Duckduckgo => {
            search_duckduckgo(query, num_results).map(|r| ("duckduckgo".to_string(), r))
        }
        SearchProvider::GoogleNewsRss => {
            search_google_news_rss(query, num_results).map(|r| ("google_news_rss".to_string(), r))
        }
        SearchProvider::BingNewsRss => {
            search_bing_news_rss(query, num_results).map(|r| ("bing_news_rss".to_string(), r))
        }
    }
}

fn search_web_auto(query: &str, num_results: usize) -> anyhow::Result<(String, Vec<SearchResult>)> {
