//! `ui.studio.gc.status` — read-only GC state and recent history.
//!
...
//! so the studio can poll/refresh GC status independently of the
//! snapshot.
...
pub async fn handle(_params: Value, ctx: HandlerContext, state: Arc<AppState>) -> Result<Value> {
    ...
    let response = GcStatusResponse {
        schema_version: "studio.gc.status.v1",
        running,
        state: gc_state,
        recent_events,
    };
