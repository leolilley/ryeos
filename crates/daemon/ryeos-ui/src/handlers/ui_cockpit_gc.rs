//! `ui.cockpit.gc.status` — read-only GC state and recent history.
//!
...
//! so the cockpit can poll/refresh GC status independently of the
//! snapshot.
...
pub async fn handle(_params: Value, ctx: HandlerContext, state: Arc<AppState>) -> Result<Value> {
    ...
    let response = GcStatusResponse {
        schema_version: "cockpit.gc.status.v1",
        running,
        state: gc_state,
        recent_events,
    };
...
pub const DESCRIPTOR: ServiceDescriptor = ServiceDescriptor {
    service_ref: "service:ui/cockpit/gc/status",
    endpoint: "ui.cockpit.gc.status",
