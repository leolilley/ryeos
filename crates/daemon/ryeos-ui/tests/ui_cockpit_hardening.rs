...
async fn browser_boot_sequence_endpoints_accept_same_session() {
    ...
    let items = (ryeos_ui::handlers::ui_cockpit_items::ITEMS_LIST_DESCRIPTOR.handler)(
        json!({
            "kind": "surface",
            "query": "ryeos/cockpit/base",
            "limit": 1
        }),
        ctx.clone(),
        state.clone(),
    ...
    .await
    .expect("items/effective should resolve cockpit surface");
    assert_eq!(effective["canonical_ref"], "surface:ryeos/cockpit/base");

    let snapshot = (ryeos_ui::handlers::ui_cockpit_snapshot::DESCRIPTOR.handler)(
        json!(null),
        ctx.clone(),
        state.clone(),
    )
    .await
    .expect("cockpit snapshot should succeed");
    assert_eq!(snapshot["schema_version"], "cockpit.snapshot.v1");

    let inspection = (ryeos_ui::handlers::ui_cockpit_items::ITEM_INSPECT_DESCRIPTOR.handler)(
        json!({
    ...
    assert_eq!(inspection["schema_version"], "cockpit.item.inspect.v1");
    assert_eq!(
...
async fn files_list_truncates_large_directories() {
    ...
    let ctx = mint_session(&state, Some(project.path().display().to_string()));
    let result = (ryeos_ui::handlers::ui_cockpit_files::FILES_LIST_DESCRIPTOR.handler)(
