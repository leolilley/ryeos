...
async fn ui_seat_open_reattaches_running_session_seat() {
    ...
    let first = (ryeos_ui::handlers::ui_studio_seat::OPEN_DESCRIPTOR.handler)(
        serde_json::json!({ "surface_ref": "surface:ryeos/studio/base" }),
        ctx.clone(),
        Arc::new(state.clone()),
    )
    .await
    .expect("open seat");
    let second = (ryeos_ui::handlers::ui_studio_seat::OPEN_DESCRIPTOR.handler)(
        serde_json::json!({ "surface_ref": "surface:ryeos/studio/base" }),
        ctx,
        Arc::new(state.clone()),
    )
...
async fn ui_seat_append_replay_and_close_round_trip() {
    ...
    let opened = (ryeos_ui::handlers::ui_studio_seat::OPEN_DESCRIPTOR.handler)(
        serde_json::json!({ "surface_ref": "surface:ryeos/studio/base" }),
        ctx.clone(),
        state.clone(),
    )
    ...
    let appended = (ryeos_ui::handlers::ui_studio_seat::APPEND_DESCRIPTOR.handler)(
        serde_json::json!({
            "thread_id": thread_id,
            "events": [{
    ...
    let replay = (ryeos_ui::handlers::ui_studio_seat::REPLAY_DESCRIPTOR.handler)(
    ...
    let closed = (ryeos_ui::handlers::ui_studio_seat::CLOSE_DESCRIPTOR.handler)(
...
async fn read_only_actions_allow_session_local_seat_services() {
    ...
    let result = (ryeos_ui::handlers::ui_actions_invoke::DESCRIPTOR.handler)(
        serde_json::json!({
            ...
            "args": { "surface_ref": "surface:ryeos/studio/base" }
        }),
        ctx,
        Arc::new(state),
    )
