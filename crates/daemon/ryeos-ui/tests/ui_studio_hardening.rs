...
async fn browser_boot_sequence_endpoints_accept_same_session() {
    ...
    let items = (ryeos_ui::handlers::ui_studio_items::ITEMS_LIST_DESCRIPTOR.handler)(
        json!({
            "kind": "surface",
            "query": "ryeos/studio/base",
            "limit": 1
        }),
        ctx.clone(),
        state.clone(),
    ...
    .await
    .expect("items/effective should resolve Studio surface");
    assert_eq!(effective["canonical_ref"], "surface:ryeos/studio/base");

    let dimension = (ryeos_ui::handlers::ui_studio_dimension::DESCRIPTOR.handler)(
        json!(null),
        ctx.clone(),
        state.clone(),
    )
    .await
    .expect("Studio dimension should succeed");
    assert_eq!(dimension["schema_version"], "ryeos.studio.dimension.v0");

    let inspection = (ryeos_ui::handlers::ui_studio_items::ITEM_INSPECT_DESCRIPTOR.handler)(
        json!({
    ...
    assert_eq!(inspection["schema_version"], "ryeos.studio.item.inspect.v1");
    assert_eq!(
...
#[tokio::test]
async fn studio_admin_lists_reject_unknown_session_ids() {
    let (_tmp, state) = test_state::build_test_state();
    let state = Arc::new(state);
    let ctx = HandlerContext::new("session:missing".into(), vec!["ui.read".into()], false);

    let threads = (ryeos_ui::handlers::ui_studio_threads::DESCRIPTOR.handler)(
        json!({"limit": 1}),
        ctx.clone(),
        state.clone(),
    )
    ...
    let schedules = (ryeos_ui::handlers::ui_studio_schedules::DESCRIPTOR.handler)(
        json!({}),
        ctx.clone(),
        state.clone(),
    )
    ...
    let gc = (ryeos_ui::handlers::ui_studio_gc::DESCRIPTOR.handler)(json!({}), ctx, state).await;
...
async fn files_read_returns_lossy_utf8_for_invalid_bytes() {
    ...
    let result = (ryeos_ui::handlers::ui_studio_files::FILES_READ_DESCRIPTOR.handler)(
...
async fn files_list_truncates_large_directories() {
    ...
    let ctx = mint_session(&state, Some(project.path().display().to_string()));
    let result = (ryeos_ui::handlers::ui_studio_files::FILES_LIST_DESCRIPTOR.handler)(
...
async fn item_inspect_raw_is_bounded_and_lossy() {
    ...
    let ctx = mint_session(&state, Some(project.path().display().to_string()));
    let result = (ryeos_ui::handlers::ui_studio_items::ITEM_INSPECT_DESCRIPTOR.handler)(
        json!({
