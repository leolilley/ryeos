//! `ui.cockpit.snapshot` — one fast initial page model for the 2D cockpit.
//!
...
//! cockpit overview without any graph interaction.
//!
...
pub async fn handle(_params: Value, ctx: HandlerContext, state: Arc<AppState>) -> Result<Value> {
    let session_id = session_id_from_context(&ctx).ok_or_else(|| {
        HandlerError::Forbidden("browser session required for cockpit snapshot".into())
    })?;
    ...
    let snapshot = build_snapshot(&state, &session, project_path.as_ref());
...
fn build_snapshot(
    ...
    Snapshot {
        schema_version: "cockpit.snapshot.v1",
...
pub const DESCRIPTOR: ServiceDescriptor = ServiceDescriptor {
    service_ref: "service:ui/cockpit/snapshot",
    endpoint: "ui.cockpit.snapshot",
