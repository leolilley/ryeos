...
async fn unknown_command_dispatched_to_session_bus() {
    ...
    let result = (ryeos_api::handlers::ui_actions_invoke::DESCRIPTOR.handler)(
        serde_json::json!({
...
async fn read_only_session_rejects_action() {
    ...
    assert!(msg.contains("read-only"), "expected read-only mention, got: {msg}");
}
...
async fn session_cookie_required() {
    ...
    let msg = format!("{:#}", result.unwrap_err());
    assert!(msg.contains("session"), "expected session mention, got: {msg}");
}
