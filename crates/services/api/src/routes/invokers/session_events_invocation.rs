...
use crate::routes::invocation::{
    ...
    RouteInvocationOutput, RouteInvocationResult, RouteEventStream,
};

use ryeos_app::session_bus::SessionBus;
...
impl CompiledRouteInvocation for CompiledSessionEventsInvocation {
    ...
    async fn invoke(
        ...
        let stream = if let Some(events) = replay {
            if events.is_empty() {
                let snap = SessionBus::snapshot_required_envelope();
                    ...
                    loop {
                        match receiver.recv().await {
                            ...
                            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
                                yield Ok(envelope_to_sse(&SessionBus::snapshot_required_envelope()));
                            }
