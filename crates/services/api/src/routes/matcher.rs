        ...
        for i in 0..entries.len() {
            for j in (i + 1)..entries.len() {
                ...
                if let Some(method) = shared_methods.into_iter().next() {
                    return Err(crate::route_error::RouteConfigError::PathCollision {
...
fn parse_path(
    pattern: &str,
    route_id: &str,
) -> Result<Vec<Segment>, crate::route_error::RouteConfigError> {
    ...
    for part in parts {
        if let Some(name) = part.strip_prefix('{').and_then(|s| s.strip_suffix('}')) {
            if name.is_empty() {
                return Err(crate::route_error::RouteConfigError::InvalidPathTemplate {
