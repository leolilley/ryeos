...
impl ThreadLifecycleClient {
    ...
    pub async fn reserve_budget(
        ...
        self.rpc
            .request(
                "budgets.reserve",
                serde_json::json!({"thread_id": thread_id, "amount": amount}),
            )
            .await
    }
    ...
    pub async fn report_budget(
        ...
        self.rpc
            .request(
                "budgets.report",
                serde_json::json!({"thread_id": thread_id, "usage": usage}),
            )
            .await
    }
    ...
    pub async fn get_budget(&self, thread_id: &str) -> Result<Value, RpcError> {
        self.rpc
            .request(
                "budgets.get",
                serde_json::json!({"thread_id": thread_id}),
