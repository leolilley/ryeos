...
Updated: 2026-09-19, Pacific/Auckland. Owner: this execution thread.
Status: RECONSTRUCTING THE QUALIFICATION PAIR — the former retained source and
target state was deleted during `/tmp` cleanup. Paths with those names were later
recreated, but contain only `.ai/.bundles.lock`; their identities, policy, grants,
project authority, product witnesses, execution history, and target HEAD are gone.
The old runit services still report their former PIDs while both endpoints are
unreachable and therefore confer no live authority. Treat every former source /
target coordinate below as historical and invalidated. No E2E pass is claimed.
...
These are historical last-known coordinates from the invalidated pair. They are
retained only as negative evidence and must not be used as current authority.

| Field | Retained value |
|---|---|
| Repo / branch | `/home/leo/projects/ryeos-next`, `next`; current HEAD `86beed305`; `origin/next` `aa57e36ce` at checkpoint time |
| Source app root | INVALIDATED `/tmp/ryeos-remote-workflow-e2e-v091.b4i6IGEs/source-node`; recreated shell contains no node state |
| Source HTTP | `http://127.0.0.1:7423` |
| Source project | INVALIDATED / absent; former detached Git `bc58d9b709194a8e13f26f641585e2266ba4ba7f`, tree `7a8ea1a83864345f5d7c2d6a8fffcb2a3b8b4e7c` |
...
| Target app root | INVALIDATED `/tmp/ryeos-remote-e2e-v091.UyjanrF8/target-node`; recreated shell contains no node state |
...
| Primary installed CLI / daemon | mixed/stale: `/usr/bin/ryeos` v0.5.93 sha256 `abd6676b5c5e8ab4749d3ce0a03aef2e882bd1f6e8721fce0e31acd33c2b2110`; `/usr/bin/ryeosd` sha256 `d18fd85f5021a1e015d273dfafc2979adce46615ce04ccb176273a25edf8db59` |
| Retained source / target daemon generation | INVALIDATED; orphan services report former PIDs 16335 / 26242, endpoints are unreachable, and backing node state is absent |
...
### G0 — reconcile and freeze prerequisites (invalidated; rebuilding)
...
| Sept 19 11:38 NZST | G0 replacement authority / target freeze | root-owned pinned v0.5.96 binaries, supported host lifecycle, source-signed base correction, origin-bound grants, project binding, and configured-operator push | both nodes healthy on release revision `91b2e70c…`; daemon executable hash `cf087705…`. The immutable release project required updated Rust/workspace assertions (`fc5e1d74…` base, `3ba5a523…` candidate, `5975e5d6…` manifest, `24f2096c…` lock); source operator signed config digest `db586d05…` and all five evaluator tests pass. Target granted the source node only forwarded-operator attestation and the source operator only five project-sync scopes. Initial push refused the absent target directory before HEAD mutation; after creating that exact bound directory, the same source state produced snapshot `6e026e9a2f3a4d522a9c8129061262433fb028ff47cc7177d047e64f53066a44` / tree `613f8c728ac56753c9e5a19bb8d66a733f1915b672c7ae9b602d9d87b2bf8bb4` (3,068 entries, 6,015 uploaded), not applied to the live worktree | freeze this HEAD and do not push again. The broad all-phase operator grant was rejected; add only phase-specific exact scopes as required. `ryeos verify` also exposed an empty retained bundle-registration error despite successful daemon-backed signing; keep it separate from the passing evaluator fixture and investigate only if it blocks installed admission |
| Sept 19 12:01 NZST | G0/G1 source-authority correction | first candidate-runtime binding against the frozen target, explicit author trust reconciliation, and narrowed executor source policy | official producer-Python activation completed in one attempt (job `external-activation:6c5c9c…`, receipt `87897be0…`, manifest `800d4969…`). Binding first refused untrusted historical project authors; stopped-node `init --trust-file` admitted only their explicit public documents while preserving target identity/state. Admission then proved `item_namespace` wrongly coupled the authoring runtime to unrelated sibling Tool directories. Added closed `item_directory` executor location, shared admission/signing selection, and an exclusion test; authoring contract suite passes 21/21, fmt and diff checks pass. Commits: contract/config `a5785634d`, substrate `f993723a8`. Corrected proof snapshot is `3b458c60079adee9551780276f50a95526c8422227cafd73fbbdb7d3fe04d24d`, tree `17d1bfdb…` (6 uploaded, 6,009 reused) | v0.5.96 correctly rejects the new location as unknown, so installed E2E pauses at this exact boundary. Let the current unrelated build finish, run focused Rust tests/review, cut the requested release containing both commits, upgrade only this retained pair, then resume the same activation/binding/product workflow without rebuilding project inputs |
