...
TOKENS_PATH = get_bundle_path(
    "standard", "tools/rye/agent/permissions/capability_tokens/capability_tokens.py"
)
...
""")
        ...
        (
            directives_dir / "test_new_perms.md"
        ).write_text("""# Test New Permission Format
...
""")
        ...
        (
            directives_dir / "test_no_perms.md"
        ).write_text("""# Test No Permissions Directive
...
class TestCapabilityChecking:
    ...
    def test_missing_capability(self):
        ...
        assert not check_capability(
            granted, "rye.execute.tool.rye.file-system.fs_write"
        )
    ...
    def test_attenuation_intersection(self):
        parent_token = CapabilityToken(
            caps=[
                ...
                "rye.execute.*",
                "rye.execute.tool.rye.agent.threads.spawn_thread",
            ],
            aud="test",
            exp=datetime.now(timezone.utc) + timedelta(hours=24),
            directive_id="parent",
